    ...
    pub fn download_track(&self, track: Track) -> Result<(), String> {
        ...
        thread::spawn(move || {
            if let Err(e) = download_worker(url, track.clone(), music_dir, app_handle.clone()) {
                let _ = app_handle.emit("download-error", format!("Failed to download {}: {}", track.title, e));
            }
...
fn download_worker(url: String, track: Track, music_dir: PathBuf, app: AppHandle) -> io::Result<()> {
