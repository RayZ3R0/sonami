...
impl AudioManager {
    ...
    pub fn pause(&self) { self.state.is_playing.store(false, Ordering::Relaxed); }
    pub fn resume(&self) { self.state.is_playing.store(true, Ordering::Relaxed); }
    pub fn stop(&self) { let _ = self.command_tx.send(DecoderCommand::Stop); }
    pub fn seek(&self, seconds: f64) { let _ = self.command_tx.send(DecoderCommand::Seek(seconds)); }
...
fn run_audio_output(buffer: Arc<AudioBuffer>, state: PlaybackState) {
    ...
    loop {
        ...
        state.device_sample_rate.store(sample_rate, Ordering::Relaxed);
    ...
    device.build_output_stream(
        ...
        move |data: &mut [T], _: &cpal::OutputCallbackInfo| {
            ...
            if read_samples > 0 {
                ...
                let ratio = if dev_rate > 0.0 { src_rate / dev_rate } else { 1.0 };
                let frames_played = read_samples / channels;
                let source_frames = (frames_played as f64 * ratio) as u64;
                state.position_samples.fetch_add(source_frames, Ordering::Relaxed);
            }
        },
        err_fn,
        None,
                                    ...
                                    if let Ok(output_frames) = r.process(&resampler_input_buffer, None) {
                                        ...
                                        if out_len > 0 {
                                            ...
                                            while written < interleaved.len() {
                                                ...
                                                if w == 0 { thread::sleep(Duration::from_micros(100)); }
                                                written += w;
                                            }
                                        }
                                    }
                         ...
                         *state.current_path.write() = Some(next_path.clone());
                         if let Ok((r, d, tid, dur, sr)) = load_track(&next_path) {
                             ...
                             if device_rate != 0 && device_rate != sr as u32 {
                                 ...
                                 if let Ok(r) = SincFixedIn::<f32>::new(device_rate as f64 / sr as f64, 2.0, params, 1024, 2) {
                                     resampler = Some(r);
                                     resampler_input_buffer = vec![vec![0.0; 1024]; 2];
                                 } else { resampler = None; }
                             } else { resampler = None; }
