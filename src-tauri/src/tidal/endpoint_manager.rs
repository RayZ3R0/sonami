...
impl EndpointManager {
    ...
    async fn load_endpoints_with_cache(cache_path: &PathBuf) -> Result<(Vec<Endpoint>, Option<StickyEndpoint>, Option<Instant>), TidalError> {
        // Try loading from cache first
        if let Ok(cache_data) = Self::load_from_cache(cache_path) {
            let age = SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_secs() - cache_data.timestamp;
            
            if age < CACHE_TTL_SECONDS {
                log::info!("Loaded {} endpoints from cache (age: {}s)", cache_data.endpoints.len(), age);
                return Ok((cache_data.endpoints, cache_data.sticky_endpoint, Some(Instant::now())));
            } else {
    ...
    async fn fetch_from_github() -> Result<Vec<Endpoint>, TidalError> {
        ...
        if !response.status().is_success() {
            return Err(TidalError::NetworkError(format!("HTTP {}", response.status())));
        }
    ...
    fn parse_instances_json(data: &serde_json::Value) -> Result<Vec<Endpoint>, TidalError> {
                ...
                if let Some(urls) = provider_data.get("urls").and_then(|v| v.as_array()) {
                    for url_value in urls {
                        if let Some(url) = url_value.as_str() {
                            ...
                            let name = url.replace("https://", "").replace("http://", "")
        ...
        if endpoints.is_empty() {
            return Err(TidalError::ParseError("No endpoints found in response".to_string()));
        }
    ...
    fn save_to_cache(path: &PathBuf, endpoints: &[Endpoint], sticky: Option<&StickyEndpoint>) -> Result<(), TidalError> {
        ...
        if let Ok(sticky_guard) = self.sticky.read() {
            if let Some(sticky) = sticky_guard.as_ref() {
                if sticky.failure_count < MAX_STICKY_FAILURES {
                    log::debug!("Using sticky endpoint: {} (failures: {})", sticky.endpoint.name, sticky.failure_count);
                    return Some(sticky.endpoint.clone());
                } else {
                    log::warn!("Sticky endpoint {} failed {} times, cycling to others", sticky.endpoint.name, sticky.failure_count);
                }
            }
        }
        ...
        if let Ok(mut sticky_guard) = self.sticky.write() {
            if let Some(sticky) = sticky_guard.as_mut() {
                if sticky.endpoint.url == endpoint.url {
                    ...
                    log::warn!("Sticky endpoint {} failed (count: {})", endpoint.name, sticky.failure_count);
                    
                    if sticky.failure_count >= MAX_STICKY_FAILURES {
                        log::warn!("Resetting sticky endpoint after {} failures", sticky.failure_count);
                        *sticky_guard = None;
                    }
                }
            }
