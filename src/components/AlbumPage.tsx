    ...
    >
        ...
        <div className="flex items-center gap-4">
            <button
                ...
                className={`opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-full hover:bg-white/10 ${isLiked
...
export const AlbumPage = ({ albumId, onNavigate }: AlbumPageProps) => {
    ...
    const handlePlayAll = useCallback(async () => {
        ...
        const queue = tracks.map(t => ({ ...t, cover_image: t.cover_image || album.cover_url }));
    ...
    const handlePlayTrack = useCallback(
        async (track: Track) => {
            if (!album) return;
            const queue = tracks.map(t => ({ ...t, cover_image: t.cover_image || album.cover_url }));
            const trackToPlay = { ...track, cover_image: track.cover_image || album.cover_url };
    ...
    const handleDownloadAll = async () => {
        ...
        for (const track of tracks) {
            const trackObj = { ...track, cover_image: track.cover_image || album.cover_url };
    ...
    const handleContextMenu = async (
        ...
        const track = { ...trackData, cover_image: trackData.cover_image || album?.cover_url };
                    ...
                    {tracks.map((track, index) => {
                        ...
                        return (
                            <TrackRow
                                ...
                                onToggleLike={(e) => {
                                    e.stopPropagation();
                                    toggleFavorite({ ...track, cover_image: track.cover_image || album.cover_url });
                                }}
                                ...
                                onDownloadClick={(e) => {
                                    ...
                                    if (isDl) {
                                        ...
                                        downloadTrack({ ...track, cover_image: track.cover_image || album.cover_url });
                                    }
                                }}
                            />
                        );
