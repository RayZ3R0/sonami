...
const ImageWithFallback = ({
  ...
  if (!src || error) {
    return (
      <div
        className={`bg-gradient-to-br from-theme-secondary to-theme-tertiary flex items-center justify-center text-theme-muted ${className}`}
      >
        {icons[iconType]}
      </div>
    );
...
const TrackCard = ({
  ...
  return (
    ...
    >
        ...
        {isPlaying && (
          <div className="absolute bottom-2 right-2 w-6 h-6 rounded-full bg-theme-accent flex items-center justify-center">
            <div className="flex items-end gap-[2px] h-3">
              <span
                className="w-[2px] bg-white rounded-full animate-eq-1"
                style={{ height: "50%" }}
              />
              <span
                className="w-[2px] bg-white rounded-full animate-eq-2"
                style={{ height: "100%" }}
              />
              <span
                className="w-[2px] bg-white rounded-full animate-eq-3"
                style={{ height: "30%" }}
              />
            </div>
          </div>
      ...
      <h3
        className={`font-semibold truncate ${isPlaying ? "text-theme-accent" : "text-theme-primary"} group-hover:text-theme-accent transition-colors`}
      >
        {track.title}
      </h3>
  ...
  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2">
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} className="p-3 animate-pulse">
        <div
          className={`aspect-square w-full bg-theme-secondary mb-3 ${isArtist ? "rounded-full" : "rounded-lg"}`}
        />
...
export const HomeView = () => {
  const { playTrack, currentTrack, addToPlaylist, playlists, toggleFavorite } =
    usePlayer();
        ...
        <div className="pt-8 pb-6">
          ...
          {!loading && !isEmpty && (
            <p className="text-theme-secondary text-lg">
              {tracks.length} tracks • {albums.length} albums • {artists.length}{" "}
              artists
            </p>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-theme-muted max-w-md mb-6">
              Import music from your computer or search Tidal to start building
              your collection.
            </p>
            <div className="flex gap-3 text-sm text-theme-muted">
              <span className="flex items-center gap-2">
                Press
                <kbd className="px-2 py-1 rounded bg-theme-secondary border border-theme-border font-mono text-xs">
                  ⌘K
                </kbd>
                to search
              </span>
