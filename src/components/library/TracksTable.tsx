          ...
          return (
            ...
            >
              <div className="relative w-full flex justify-center text-theme-muted font-mono text-sm">
                <span
                  className={`group-hover:opacity-0 transition-opacity flex items-center justify-center h-5 w-5 ${isPlaying ? "text-theme-accent" : ""}`}
                >
                  {isPlaying ? (
              ...
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="flex flex-col truncate pr-2">
                  <span
                    className={`truncate font-medium text-[15px] ${isPlaying ? "text-theme-accent" : "text-theme-primary"}`}
                  >
                    {track.title}
                  </span>
                  {track.source === "TIDAL" && (
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-[9px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-1 rounded uppercase tracking-wider">
                        TIDAL
                      </span>
                    </div>
                  )}
