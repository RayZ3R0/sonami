...
export const SearchPalette = ({ isOpen, onClose }: SearchPaletteProps) => {
    const { playTrack, playlists, addToPlaylist, toggleFavorite, refreshFavorites } = usePlayer();
    ...
    useEffect(() => {
        ...
        const searchTidal = async () => {
            ...
            try {
                ...
                const response: any = await invoke('tidal_search_tracks', { query: activeQuery });
    ...
    const getTrackFromResult = (result: SearchResult) => {
        ...
        return {
            ...
            cover_image: t.album?.cover ? `https://resources.tidal.com/images/${t.album.cover.replace(/-/g, '/')}/640x640.jpg` : undefined,
            path: `tidal:${t.id}`,
            tidal_id: t.id,
            source: 'TIDAL'
