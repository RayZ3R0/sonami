    ...
    return (
        ...
        >
            <div className={`inline-flex ${isOverflowing ? 'animate-marquee' : ''}`}>
                <span ref={textRef} className={`${className} ${isOverflowing ? 'pr-8' : ''}`}>
                    {text}
                </span>
                {isOverflowing && (
                    <span className={`${className} pr-8`}>
                        {text}
                    </span>
                )}
            </div>
...
const ShuffleIcon = ({ active }: { active: boolean }) => (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={active ? "text-theme-accent" : ""}>
...
const RepeatIcon = ({ mode }: { mode: "off" | "all" | "one" }) => (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={mode !== "off" ? "text-theme-accent" : ""}>
...
export const PlayerBar = () => {
    const { currentTrack, isPlaying, togglePlay, seek, volume, setVolume, nextTrack, prevTrack, shuffle, repeatMode, toggleShuffle, toggleRepeat, isQueueOpen, setIsQueueOpen, playerBarStyle } = usePlayer();
                    ...
                    <div className="px-4 pt-3 pb-2 flex items-center justify-between relative z-10">
                        ...
                        <div className="flex items-center justify-end w-[30%] gap-3 pr-1">
                            <div className="group flex items-center gap-2">
                                ...
                                >
                                    <div className="h-full progress-fill" style={{ width: `${volume * 100}%` }} />
                                </div>
                            </div>
