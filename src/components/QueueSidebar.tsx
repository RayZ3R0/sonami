...
const QueueTrackItem = ({
  ...
  >
    <span className="w-6 text-xs text-theme-muted text-center flex-shrink-0">
      {isCurrent ? (
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          className="text-theme-accent mx-auto"
        >
    ...
    <div className="flex-1 min-w-0">
      <p
        className={`text-sm truncate ${isCurrent ? "text-theme-accent font-medium" : "text-theme-primary"}`}
      >
        {track.title}
      </p>
...
export const QueueSidebar = () => {
  ...
  const currentIndex = currentTrack
    ...
    : -1;
  const upcomingLibraryTracks =
    currentIndex >= 0 ? tracks.slice(currentIndex + 1, currentIndex + 20) : [];
