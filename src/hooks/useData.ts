...
const normalizeAlbum = (album: Album, providerId: string): Album => {
    ...
    return {
        ...
        artist_id: album.artist_id && !album.artist_id.includes(":") ? `${providerId}:${album.artist_id}` : album.artist_id
    };
}
            ...
            try {
                ...
                if (providerId === "local") {
                    ...
                    [{ val: albumData }, { val: tracksData }] = await Promise.all([
                        invoke("get_album", { providerId, albumId: id }).then((v: any) => ({ val: v })),
                        invoke("get_album_tracks", { providerId, albumId: id }).then((v: any) => ({ val: v })),
                    ]);
                }
    ...
    useEffect(() => {
        const fetchArtist = async () => {
            ...
            try {
                ...
                if (providerId === "local") {
                    console.log("[useArtist] 'local' provider detected. Checking fallbacks:", id);
                    ...
                    for (const p of fallbackProviders) {
                        try {
                            const [a, t, al] = await Promise.all([
                                ...
                                invoke<Track[]>("get_artist_top_tracks", { providerId: p, artistId: id }),
                                invoke<Album[]>("get_artist_albums", { providerId: p, artistId: id }),
                            ]);
                            artistData = a;
                            topTracksData = t;
                            albumsData = al;
                    ...
                    if (!found) {
                        throw new Error("Artist not found (Local provider not implemented and fallbacks failed)");
                    }
                } else {
                    [{ val: artistData }, { val: topTracksData }, { val: albumsData }] = await Promise.all([
                        invoke("get_artist", { providerId, artistId: id }).then((v: any) => ({ val: v })),
                        invoke("get_artist_top_tracks", { providerId, artistId: id }).then((v: any) => ({ val: v })),
                        invoke("get_artist_albums", { providerId, artistId: id }).then((v: any) => ({ val: v })),
                    ]);
                }
                ...
                setTopTracks(topTracksData!.map(t => normalizeTrack(t, usedProviderId)));
